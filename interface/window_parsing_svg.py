...
from config.general_functions import (check_directory, check_file,
                                      ...
                                      loading_data_dict_kks_ana, loading_data_dict_kks_bin, loading_data_dict_kks_nary,
                                      sort_files_into_groups)
...
class ParsingSvg(QMainWindow):
    def __init__(self, main_menu):  # изменим начальные настройки
        ...
        self.name_system_sorting_comments = NameSystemWindow(func=self.checking_svg_files,
    ...
    async def sorting_notes_files(self, name_directory: str) -> None:
        ...
        if name_directory != '0':
            ...
            if len(vis_groups):
                ...
                await self.print_log(text='Распределено успешно!', color='green')
            else:
                await self.print_log(text='Распределение невозможно!', color='red')
