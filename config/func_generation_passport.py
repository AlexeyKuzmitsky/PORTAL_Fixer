...
async def program_new_passport(name_system: str, print_log, progress: QProgressBar):
    print_log('Проверка наличия папки с скриншотами видеокадров')
    ...
    for name_svg in list_name_svg[num:]:
        if not name_svg.endswith('.svg'):
            print_log(text=f'Найден файл {name_svg} не являющийся svg файлом', color='yellow')
            continue
        num += 1
        print_log(text=f'[{num}/{number_name_svg}] Проверка {name_svg}\t')

        list_submodel = video_frame_parsing(svg=name_svg, name_system=name_system, print_log=print_log)


def video_frame_parsing(svg: str, name_system: str, print_log) -> List[AnchorPoint]:
    """
    ...
    with open(path.join(name_system, 'NPP_models', f'{svg}'), encoding='windows-1251') as svg_file:
        ...
        if not img:
            print_log(text=f'Нет скриншота видеокадра {svg[:-4]}', a_new_line=False, color='red')
    ...
    if path.isfile(path.join(name_system, 'screen', f'{svg[:-3]}jpg')):
        ...
        print_log(text=f'Файл {path.join(name_system, "screen", f"{svg[:-3]}jpg")} не найден', color='red')
